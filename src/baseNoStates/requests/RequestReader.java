...
public class RequestReader implements Request {
  ...
  private void authorize(User user, Door door) {
    if (user == null) {
      ...
      if (user.validateArea(door.getFrom().getId())){ //comprovem si l'usuari pot estar a l'area en la que es troba
        if(user.validateArea(door.getTo().getId())){ //comprovem si l'usuari pot estar a l'àrea en la que vol accedir
          if (user.validateActions(action)){ //comprovem que l'acció que vol realitzar la pot fer
            if (user.validateDate(now)) //validem que estigui realitzant l'accio en l'horari que se li és permès
              authorized = true;
            else {
              authorized = false;
              addReason("You can't do any actions in this date: " + now);
            }
          } else {
            authorized = false;
            addReason("You can't do this action: " + action);
          }
        } else {
          authorized = false;
          addReason("You have no access to "+ door.getTo().getId());
        }
      } else {
        authorized = false;
        addReason("You have no access to "+ door.getFrom().getId());
      }
    }
